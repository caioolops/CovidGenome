...
int main() {
    ifstream entrada("C:/Users/caiol/OneDrive/Documentos/CovidGenome/genome.txt");
    ...
    while (getline(entrada, linha)) {
        if (linha.length() < 6) continue;  // Ignora linhas menores que 6 caracteres
        if (linha[0] == '>') continue;    // Ignora linhas de cabeçalho (que começam com '>')

        // Pega os blocos de 6 caracteres
        for (size_t i = 0; i + 6 <= linha.length(); i++) {
            string bloco = linha.substr(i, 6);
            tabela.inserir(bloco);  // Conta as ocorrências dos blocos
        }
